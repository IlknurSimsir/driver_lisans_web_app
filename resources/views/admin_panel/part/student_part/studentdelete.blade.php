    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="deleteModalLabel">Öğrenci Sil</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <form id="deleteForm" action="{{ route('studentdelete') }}" method="POST">
                    @csrf
                    <!-- @method('PUT') alanını kaldırıyoruz çünkü sadece POST kullanacağız -->
                    <input type="hidden" name="id" id="deleteId">
                    <div class="mb-3">
                        <label for="deleteName" class="form-label">İsim</label>
                        <input type="text" class="form-control" id="deleteName" name="name" disabled>
                    </div>
                    <div class="mb-3">
                        <label for="deleteSurname" class="form-label">Soyisim</label>
                        <input type="text" class="form-control" id="deleteSurname" name="surname" disabled>
                    </div>
                    <center>
                        <h6>Öğrenciyi silmek istediğinizden emin misiniz?</h6>
